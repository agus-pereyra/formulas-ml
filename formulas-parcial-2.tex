...
\begin{tcolorbox}[formulabox={Demo: $\mathcal{L}$ VAE}, colframe=softBrown]
    Para cualquier $q(\z)$, multiplicar y dividir por $q(\z)$:
    \begin{align*}
    \ln p(\x|\w) &= \ln\sum_\z p(\x,\z|\w) \\
    &= \ln\sum_\z q(\z)\frac{p(\x,\z|\w)}{q(\z)}
    \end{align*}
    Separar en dos términos usando $\ln\tfrac{p(\x,\z)}{q} = \ln\tfrac{p(\x|\w) p(\z|\x,\w)}{q}$:
    $$= \underbrace{\sum_\z q(\z)\ln\frac{p(\x,\z|\w)}{q(\z)}}_{\mathcal{L}(q,\w)}
      + \underbrace{\sum_\z q(\z)\ln\frac{q(\z)}{p(\z|\x,\w)}}_{\text{KL}(q\|p(\z|\x,\w))}$$
    Como KL $\geq 0$: $\ln p(\x|\w) \geq \mathcal{L}(q,\w)$ ($\mathcal{L}$ es cota inf.)
\end{tcolorbox}
